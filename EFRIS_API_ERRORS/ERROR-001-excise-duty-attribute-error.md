...
The issue was a field name mismatch. The code was accessing `company.is_test_mode`, but the actual database field is `company.efris_test_mode`.

### What Was Fixed
Updated 6 external API endpoints in `api_multitenant.py` to use the correct field name:

1. **Line 6904** - `POST /api/external/efris/submit-invoice`
2. **Line 7129** - `POST /api/external/efris/register-product`
3. **Line 7201** - `POST /api/external/efris/submit-purchase-order`
4. **Line 7327** - `POST /api/external/efris/submit-credit-note`
5. **Line 7579** - `GET /api/external/efris/excise-duty`
6. **Line 7726** - `POST /api/external/efris/stock-decrease`
...
- ✅ No more references to `company.is_test_mode` found
- ✅ Changes committed to git (commit pending)
- ⏳ Production deployment pending

### Next Steps
1. **Push to production server** - Deploy the updated `api_multitenant.py`
2. **Test the endpoint** - Retry `GET /api/external/efris/excise-duty`
3. **Verify all endpoints** - Test the other 5 fixed endpoints
4. **Close error report** - Mark as resolved once production is working
